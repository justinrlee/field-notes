...
public class CLConsumerPerformance {
    private static final Logger LOG = LoggerFactory.getLogger(CLConsumerPerformance.class);
    ...
    private static void consume(KafkaConsumer<byte[], byte[]> consumer,
        ...
        while (messagesRead < numMessages && currentTimeMs - lastConsumedTimeMs <= recordFetchTimeoutMs) {
            ...
            for (ConsumerRecord<byte[], byte[]> record : records) {
                messagesRead += 1;
                // System.out.println(record.headers().lastHeader("timestamp"));
                // System.out.println(record.headers().lastHeader("timestamp").value());
                buffer.clear();
